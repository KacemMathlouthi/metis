...
def upgrade() -> None:
    ...
    sa.Column('github_id', sa.Integer(), nullable=False, comment='GitHub user ID (immutable)'),
    sa.Column('username', sa.String(length=255), nullable=False),
    sa.Column('email', sa.String(length=255), nullable=True),
    sa.Column('avatar_url', sa.String(length=500), nullable=True),
    sa.Column('access_token', sa.String(length=500), nullable=False, comment='Encrypted GitHub OAuth token'),
    sa.Column('refresh_token', sa.String(length=500), nullable=True, comment='Encrypted refresh token'),
    ...
    sa.Column('github_installation_id', sa.Integer(), nullable=False, comment='GitHub App installation ID'),
    sa.Column('user_id', sa.UUID(), nullable=False),
    sa.Column('account_type', sa.Enum('USER', 'ORGANIZATION', name='account_type_enum'), nullable=False, comment='Whether installed on user account or org'),
    sa.Column('account_name', sa.String(length=255), nullable=False),
    sa.Column('repository', sa.String(length=500), nullable=False, comment="Repository in format 'owner/repo'"),
    sa.Column('config', postgresql.JSONB(astext_type=sa.Text()), nullable=False, comment='ReviewerConfig as JSON: sensitivity, custom_instructions, etc.'),
    sa.Column('is_active', sa.Boolean(), nullable=False),
    ...
    op.create_index(op.f('ix_installations_account_name'), 'installations', ['account_name'], unique=False)
    ...
    op.create_index(op.f('ix_installations_repository'), 'installations', ['repository'], unique=False)
    ...
    sa.Column('commit_sha', sa.String(length=40), nullable=False, comment='Git commit SHA being reviewed'),
    sa.Column('status', sa.Enum('PENDING', 'PROCESSING', 'COMPLETED', 'FAILED', name='review_status_enum'), nullable=False),
    sa.Column('review_text', sa.Text(), nullable=True, comment='Overall review summary'),
    sa.Column('pr_metadata', postgresql.JSONB(astext_type=sa.Text()), nullable=True, comment='PR title, author, description, file count, etc.'),
    sa.Column('started_at', sa.DateTime(timezone=True), nullable=True),
    sa.Column('completed_at', sa.DateTime(timezone=True), nullable=True),
    sa.Column('error', sa.Text(), nullable=True, comment='Error message if status=FAILED'),
    sa.Column('github_review_id', sa.Integer(), nullable=True, comment='GitHub API review ID'),
    sa.Column('id', sa.UUID(), nullable=False),
    sa.Column('created_at', sa.DateTime(timezone=True), nullable=False),
    sa.Column('updated_at', sa.DateTime(timezone=True), nullable=False),
    ...
    op.create_index(op.f('ix_reviews_installation_id'), 'reviews', ['installation_id'], unique=False)
    ...
    sa.Column('category', sa.Enum('BUG', 'SECURITY', 'PERFORMANCE', 'STYLE', 'MAINTAINABILITY', 'DOCUMENTATION', 'TESTING', name='category_enum'), nullable=False),
    sa.Column('github_comment_id', sa.Integer(), nullable=True, comment='GitHub API comment ID'),
    sa.Column('id', sa.UUID(), nullable=False),
    sa.Column('created_at', sa.DateTime(timezone=True), nullable=False),
    sa.Column('updated_at', sa.DateTime(timezone=True), nullable=False),
    ...
    op.create_index(op.f('ix_review_comments_category'), 'review_comments', ['category'], unique=False)
